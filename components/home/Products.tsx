...
function ProductCard({ card, id }: { card: HeroSection; id: number }) {
  ...
  return (
    <div className="group  relative aspect-5/5 md:aspect-3/5 w-full overflow-hidden bg-zinc-100">
      <Image
        src={card.image}
        alt={card.title}
        fill
        className="object-cover h-32 md:h-auto transition-transform duration-500 group-hover:scale-[1.03]"
      />
  ...
  return (
    <section className="w-full bg-white">
      <div className="mx-auto max-w-7xl px-4 py-8 md:py-16">
        <h2 className="text-2xl font-semibold uppercase tracking-[0.08em] text-black md:text-4xl">
          Integrated Fancy Yarn & Apparel Solutions
        </h2>

        <p className="mt-4 max-w-3xl text-[16px] leading-8 text-zinc-800">
          We specialized expertise in fancy yarns, hosiery, sweaters, knitted
          apparel, and seamless activewear; serving diverse markets across all
          ages and genders with innovative textile and apparel solutions.
        </p>

        <div className="mt-6 md:mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {allProducts?.map((c) => (
